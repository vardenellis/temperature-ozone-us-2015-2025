"""Run the authorized frozen real continuous fit and point decomposition."""
...
from varden_ozone.primary_continuous import (
    ...
    load_authorized_primary_population,
    load_gaussian_fit,
...
def _fit_report(
    ...
    return f"""# Frozen continuous primary-fit report
...
def run(panel: Path, analysis_dir: Path, report_dir: Path) -> None:
    ...
    command = (
        ".venv/bin/python scripts/run_primary_continuous_fit.py "
        "--panel data/processed/site_day_panel.parquet"
    )
    started_at = datetime.now(UTC)
    wall_start = time.perf_counter()
    population = load_authorized_primary_population(panel)
    ...
    pd.DataFrame(records).to_csv(
        analysis_dir / "decomposition_point_estimates.csv",
        index=False,
    )
    _write_json(analysis_dir / "decomposition_point_estimates.json", records)
    reproducibility = {
        "fit_repeat": repeat,
    ...
    (report_dir / "primary_fit_report.md").write_text(
    ...
    (report_dir / "decomposition_point_estimate_report.md").write_text(
        _decomposition_report(records),
        encoding="utf-8",
    )
    (report_dir / "reproducibility_report.md").write_text(
...
def main() -> None:
    ...
    parser.add_argument(
        "--analysis-dir",
        type=Path,
        default=Path("outputs/analysis/primary_continuous"),
    )
    parser.add_argument(
        "--report-dir",
        type=Path,
        default=Path("outputs/reports/primary_continuous"),
    )
