"""Restartable whole-site bootstrap for the frozen broader network."""
...
from varden_ozone.network_breadth import (
    EXPECTED_PANEL_SHA256,
    EXPECTED_PANEL_SIZE,
    EXPECTED_POPULATION_SHA256,
    STRUCTURAL_COLUMNS,
    construct_network_population,
    select_eligible_network_rows,
)
...
EXPECTED_PRE_SUPPORT_ROWS = 2_838_130
...
BASE_COLUMNS = STRUCTURAL_COLUMNS
...
def load_bootstrap_source(panel_path: Path) -> BootstrapSource:
    """Load the frozen 1,116-site pre-support rows and real continuous outcome."""
    ...
    point_view, _audit, structural = construct_network_population(panel_path)
    if point_view.identity.population_sha256 != EXPECTED_POPULATION_SHA256:
        raise ValueError("network point population checksum changed")
    panel = structural
    panel["_panel_row"] = np.arange(len(panel), dtype=np.int64)
    frame, eligible_sites = select_eligible_network_rows(panel)
    if len(eligible_sites) != EXPECTED_SITES:
        raise ValueError("network source-site eligibility changed")
    outcome = pq.read_table(panel_path, columns=["ozone_mda8_ppb"]).to_pandas()
    frame["ozone_mda8_ppb"] = outcome["ozone_mda8_ppb"].to_numpy(float)[
        frame["_panel_row"].to_numpy(np.int64)
    ]
